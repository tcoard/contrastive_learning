...
from util import TwoCropTransform, AverageMeter, BSWorkAround, CoalaDataset
from util import adjust_learning_rate, warmup_learning_rate
from util import set_optimizer, save_model
from networks.resnet_big import SupConResNet
from networks.nn_modules import SingleModel
...
def parse_option():
    ...
    parser.add_argument(
        "--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100", "path"], help="dataset"
    )
    ...
    opt.model_name = "{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}".format(
        opt.method, opt.dataset, opt.model, opt.learning_rate, opt.weight_decay, opt.batch_size, opt.temp, opt.trial
    )
    ...
    if opt.warm:
        ...
        if opt.cosine:
            ...
            opt.warmup_to = (
                eta_min + (opt.learning_rate - eta_min) * (1 + math.cos(math.pi * opt.warm_epochs / opt.epochs)) / 2
            )
        else:
...
def set_loader(opt):
    ...
    #     return seq
    def name_loader(path):
        with open(path, "r") as f:
            data = next(f).strip()
            return data

    def not_a_transform(x):
        return x
    ...
    test_dataset = CoalaDataset(root_dir=opt.data_folder, run_type="test")
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        if (idx + 1) % opt.print_freq == 0:
            ...
            #     ((name, sys.getsizeof(value)) for name, value in locals().items()), key=lambda x: -x[1]
            # )[:10]:
