...
class ResNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, bias=False, n_layers=4):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.ResLayer_list = nn.ModuleList(
            [ResidualBlock(self.n_channels, kernel_size, stride, bias) for i in range(n_layers)]
        )
    ...
    def __init__(
        ...
        if use_gru:
            self.bilstm = nn.GRU(
                ...
                n_hidden_state,
                n_lstm_layers,
                dropout=(0 if n_lstm_layers == 1 else lstm_dropout),
                bidirectional=True,
                batch_first=True,
            )
        else:
...
class SingleModel(nn.Module):
    ...
    def __init__(
        ...
        self.n_hidden_state = n_hidden_state
        self.att_bilstm_1 = LearningModule(
            in_channels, out_channels, kernel_size, n_layers, n_hidden_state, use_gru, lstm_dropout, n_lstm_layers
        )
